...
float eq_nroot(float x, float n) {
  float ni = floor(n + 0.5);
  if (abs(n) < 0.5 || abs(n - ni) > 0.00001 || (x < 0.0 && mod(abs(ni), 2.0) < 0.5)) return sqrt(-1.0);
  return sign(x) * pow(abs(x), 1.0 / ni);
}
