...
export function lookAt(eye: number[], target: number[], up: number[]): Mat4 {
  ...
  // The preferred up vector is parallel to a pole-on camera direction at
  // phi = ±pi/2. Switch to a stable alternate up vector instead of allowing
  // NaNs into every projected vertex.
  if (xl < 1e-12) {
    const fallback = [0, 1, 0];
    xx = fallback[1] * zz - fallback[2] * zy;
    xy = fallback[2] * zx - fallback[0] * zz;
    xz = fallback[0] * zy - fallback[1] * zx;
